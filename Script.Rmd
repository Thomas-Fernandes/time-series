...
Ce qui est intéressant avec ce processus, c'est qu'il est déterministe. En connaissant le $A$, $B$ et $\omega$ de départ, on peut déterminer toute la trajectoire de $X_t$. Cependant, notre processus est défini pour $\omega$ fixé dans $[0, 2\pi]$ et simulé sur n = 200. Or, si je prends un $\omega$ proche de $0$ mais $\ne0$, la trajectoire de $X_t$ s'apparentera à celle d'un processus non-stationnaire.
...
1 est racine évidente du polynôme caractéristique $r = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$.
...
```{r}
x <- sarima(p1~0 | ma(1) + ar(2) + sar(52,1) + i(1) + sma(1))
```


```{r}
set.seed(123)
x <- sim_sarima(n = 500, # nombre de points de données
...
On remarque que qu'il y a toujours une saisonalité sur Yt, mais qu'elle est plus dans un ordre 5 ou.